...
def parseInputStmt():
    ...
    match(TokenType.LPAREN)
    if currentToken == TokenType.ID:
        node.name = currentLexeme
        nextToken(False)
    else:
        syntaxError("se esperaba identificador en input")
    match(TokenType.RPAREN)
    match(TokenType.SEMI)
    return node
...
def parseFactor():
    if currentToken == TokenType.LPAREN:
        ...
        nextToken(False)
        return node
    elif currentToken in {TokenType.ID, TokenType.INPUT, TokenType.OUTPUT}:
        name = currentLexeme
        nextToken(False)
        if currentToken == TokenType.LPAREN:
            nextToken(False)
            ...
            node.child[0] = args
            return node
        # si no hay paréntesis, lo tratamos como un ID normal
